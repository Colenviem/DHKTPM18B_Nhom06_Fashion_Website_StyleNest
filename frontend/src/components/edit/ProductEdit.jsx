  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        ...
        alert("Không thể tải chi tiết sản phẩm hoặc danh mục!");
...
    console.log("--- End Dữ liệu Sản phẩm ---");
    try {
      setSaving(true);
      const updatedProduct = await saveOrUpdateProduct(product); 
  ...
  return (
    <motion.div className="p-6 pt-24 bg-gray-50 min-h-screen" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="flex justify-between items-center mb-6 sticky top-0 bg-gray-50/90 z-10 py-4 border-b">
        <h1 className="text-3xl font-bold">Chỉnh Sửa: {product.name || "Sản phẩm mới"}</h1>
                ...
                return (
                  <div key={field} className="relative flex items-center">
                    <input
                      ...
                      className={`w-full border p-2 rounded-lg pr-10 transition ${
                        isChanged
