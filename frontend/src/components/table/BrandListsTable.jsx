...
const getBrandStatus = (isActive) =>
  isActive
    ...
    : { text: "Ngừng hoạt động", icon: <FiXCircle className="w-5 h-5 text-red-500 mx-auto" />, colorClass: "text-red-600 font-bold" };
...
const BrandListsTable = () => {
  ...
  const [formData, setFormData] = useState({ name: "", description: "", logoUrl: "", isActive: true, isFeatured: false });
        ...
        <div className="relative flex items-center w-full sm:w-96 gap-2">
          <div className="relative flex-grow">
            ...
            <input
              ...
              onKeyDown={(e) => e.key === "Enter" && setSearchKeyword(searchInput.trim())}
          ...
          >
            {searchLoading ? <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div> : <><FiSearch className="w-4 h-4" />Tìm</>}
          </button>
        </div>
        <button
      ...
      {brandsData.map((brand) => {
        ...
        return (
          <motion.tr key={brand.id} variants={rowVariants} className="hover:bg-gray-50 transition-colors">
            ...
            {/* Ngày tạo */}
            <td className="px-6 py-4 text-gray-500 whitespace-nowrap text-xs font-mono text-center">{formatDateTime(brand.createdAt)}</td>
            ...
            {/* Trạng thái */}
            <td className="px-6 py-4 text-center whitespace-nowrap">
              <div className="flex flex-col items-center justify-center gap-1">{status.icon}<span className={status.colorClass}>{status.text}</span></div>
            </td>

            {/* Hành động */}
            <td className="px-6 py-4 text-center whitespace-nowrap">
              <div className="flex justify-center items-center gap-2">
                <button title="Sửa" className="p-2 text-indigo-600 hover:text-indigo-800 rounded-full hover:bg-indigo-100 transition-colors" onClick={() => openModal(brand)}>
      ...
      {brandsData.length === 0 && (
        <tr>
          <td colSpan="7" className="text-center py-8 text-gray-500 italic">
            {searchKeyword ? `Không tìm thấy thương hiệu nào phù hợp với từ khóa "${searchKeyword}".` : "Không có thương hiệu nào."}
          </td>
        </tr>
      )}
    </motion.tbody>
