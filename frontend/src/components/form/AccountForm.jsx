...
const AccountForm = () => {
  ...
  // =========================================================
  //        FETCH ACCOUNT + USER nếu có nhận được id từ URL (chế độ chỉnh sửa)
  // =========================================================

  useEffect(() => {
    if (!id) return; // Không có id → chế độ Add
    ...
    const fetchData = async () => {
      try {
        ...
        // 2. Fetch user theo userId
        if (acc.userId) {
          const userRes = await axios.get(
            `http://localhost:8080/api/users/${acc.userId}`,
            {
              headers: { Authorization: `Bearer ${token}` },
            }
          );
          ...
          setUser({
            ...
            addresses: usr.addresses?.length
              ? usr.addresses.map((address) => ({
                  ...
                  isDefault: Boolean(address.isDefault), // chuẩn hóa boolean
                }))
              : [
                  {
                    id: uuidv4(),
  ...
  // Kiểm tra và đặt địa chỉ mặc định
