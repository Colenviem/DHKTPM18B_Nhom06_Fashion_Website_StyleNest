...
const AddCouponForm = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "expirationDate") {
      ...
      setErrors((prev) => ({
        ...prev,
        expirationError:
            selectedDate < now ? "Ngày hết hạn phải lớn hơn hoặc bằng hiện tại" : "",
      }));
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(error);
      const errorMsg = error.response?.data?.message || "Thêm coupon thất bại!";
      alert(`❌ ${errorMsg}`);
    }
  };
              ...
              <div className="relative bg-white text-gray-900 rounded-xl shadow-xl p-6 w-full max-w-md z-50 transition-transform transform scale-95 animate-fadeIn">
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    {errors.codeError && (
                        <span className="text-red-500 text-sm">{errors.codeError}</span>
                    )}
                  </div>
                  ...
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center gap-2">
                      ...
                      <input
                          ...
                          onChange={handleChange}
                          className={`flex-1 border px-3 py-2 rounded focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                              errors.expirationError ? "border-red-500" : "border-gray-300"
                          }`}
                          required
                      />
                    </div>
